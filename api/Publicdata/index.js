...
const TYPE_MAP = {
  stock: { path: '/get_stock_data', queryName: 'ticker', idPattern: /^[A-Za-z0-9._:\-^=]{1,80}$/ },
...
function jsonResponse(status, body, headers = {}) {
  return { status, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'public, max-age=300', ...headers }, body: JSON.stringify(body) };
}
function getParam(req, name) { return req.query?.[name] || req.params?.[name] || ''; }
...
module.exports = async function (context, req) {
  ...
  if (!config) { context.res = jsonResponse(400, { error: 'Neplatny typ dat.', type }); return; }
  let normalizedId = '';
  if (config.queryName) {
    if (!id || id.length > MAX_ID_LENGTH || !config.idPattern.test(id)) {
      context.res = jsonResponse(400, { error: 'Neplatny identifikator.' }); return;
    }
  ...
  if (hit && hit.expiresAt > now) { context.res = jsonResponse(200, hit.data, { 'x-proxy-cache': 'HIT' }); return; }
  const target = new URL(INTERNAL_APIM_BASE_URL + config.path);
  if (config.queryName) target.searchParams.set(config.queryName, normalizedId);
  const headers = { accept: 'application/json' };
  if (INTERNAL_APIM_SUBSCRIPTION_KEY) headers['Ocp-Apim-Subscription-Key'] = INTERNAL_APIM_SUBSCRIPTION_KEY;
  try {
    const data = await fetchJson(target.toString(), headers);
    memoryCache.set(cacheKey, { data, cachedAt: now, expiresAt: now + CACHE_TTL_SECONDS * 1000 });
    context.res = jsonResponse(200, data, { 'x-proxy-cache': 'MISS' });
  } catch (err) {
    context.log.error('[public-data] backend error', err.message);
    if (hit && hit.data) { context.res = jsonResponse(200, hit.data, { 'x-proxy-cache': 'STALE', 'cache-control': 'public, max-age=60' }); return; }
