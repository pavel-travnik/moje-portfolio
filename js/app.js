...
if (!page || page === "undefined") {
 ...
 main.innerHTML = ''; // bezpečné
    ...
    // ===============================
    // VYHLEDÁVÁNÍ NAPŘÍČ INSTRUMENTY
    // ===============================
    if (page === 'vyhledavani') {
        renderInstrumentSearchPage();
        trackCurrentPage(page);
        if (pushState) history.pushState({ page }, '', '/vyhledavani');
        return;
    }
    // ===============================
    // STANDARD PAGE LOAD
...
const main = document.getElementById('mainContent');
 if (!main) return;
            ...
            if (page === 'uvod') initHomeMarketOverview();
...
// HOME INDEXY + GLOBALNI VYHLEDAVANI v47
const HOME_INDEX_PREFERENCES=['^FTSE','^DJI','^IXIC','^GDAXI'];
const HOME_MARKET_CACHE_KEY='home_market_overview_v47';
const HOME_MARKET_CACHE_TTL_MS=10*60*1000;
...
function miniIndexSvg(rows){const a=(rows||[]).map(x=>Number(x.close)).filter(Number.isFinite);if(a.length<2)return '<div class="market-sparkline-empty">Historie není dostupná</div>';const w=320,h=92,p=5,min=Math.min(...a),max=Math.max(...a),range=max-min||1;const pts=a.map((v,i)=>`${(p+i/Math.max(1,a.length-1)*(w-2*p)).toFixed(1)},${(p+(max-v)/range*(h-2*p)).toFixed(1)}`).join(' ');return `<svg class="market-sparkline" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-hidden="true"><polyline points="${pts}"></polyline></svg>`;}
function readHomeMarketCache(){try{const raw=localStorage.getItem(HOME_MARKET_CACHE_KEY);if(!raw)return null;const item=JSON.parse(raw);return item&&Array.isArray(item.cards)&&Number.isFinite(Number(item.cachedAt))?item:null;}catch{return null;}}
function writeHomeMarketCache(cards){try{localStorage.setItem(HOME_MARKET_CACHE_KEY,JSON.stringify({cachedAt:Date.now(),cards}));}catch(e){console.warn('Home market cache write skipped:',e);}}
function renderHomeMarketCards(grid,cards){if(!grid||!cards?.length)return false;grid.innerHTML=cards.map(({item,rows,last,change})=>{const detail=`indexy/${encodeURIComponent(item.ticker)}`,value=last&&Number.isFinite(Number(last.close))?Number(last.close).toLocaleString('cs-CZ',{maximumFractionDigits:2}):'—',date=last?.date?new Date(last.date).toLocaleDateString('cs-CZ'):'—',cls=change===null?'':change>=0?'pos':'neg',ct=change===null?'—':`${change>=0?'+':''}${Number(change).toLocaleString('cs-CZ',{minimumFractionDigits:2,maximumFractionDigits:2})} %`;return `<a class="market-index-card" href="/${detail}" data-page="${detail}"><div class="market-index-head"><div><strong>${escapeHtml(item.name||item.ticker)}</strong><small>${escapeHtml(item.ticker)}</small></div><span class="market-index-change ${cls}">${ct}</span></div>${miniIndexSvg(rows)}<div class="market-index-foot"><strong>${value} ${escapeHtml(last?.currency||item.currency||'')}</strong><small>${date}</small></div></a>`}).join('');grid.dataset.loaded='true';return true;}
async function fetchHomeMarketCards(forceRefresh=false){await ensureStockUniverse();const indexes=selectHomeIndexes(apiCache.indexesList,4);return Promise.all(indexes.map(async item=>{try{let history;if(forceRefresh){history=await cachedJsonFetch(publicDataProxyUrl('stock',item.ticker),{forceRefresh:true,ttlMs:HOME_MARKET_CACHE_TTL_MS});if(Array.isArray(history)){history.sort((a,b)=>new Date(a.date)-new Date(b.date));apiCache.stocks[item.ticker]=history;}}else history=await ensureStockHistory(item.ticker);const f=filterPeriod(history,'1Y'),rows=(f.length?f:history).slice(-260),last=rows.at(-1),prev=rows.at(-2),change=last&&prev&&Number(prev.close)?(Number(last.close)/Number(prev.close)-1)*100:null;return{item,rows,last,change};}catch(e){console.warn('Index load failed',item.ticker,e);return{item,rows:[],last:null,change:null};}}));}
async function initHomeMarketOverview(){const main=document.getElementById('mainContent');if(!main)return;const section=main.querySelector('#home-market-overview');if(!section)return;const grid=section.querySelector('#home-market-grid');if(!grid)return;const cached=readHomeMarketCache(),fresh=cached&&Date.now()-Number(cached.cachedAt)<HOME_MARKET_CACHE_TTL_MS;if(cached)renderHomeMarketCards(grid,cached.cards);if(!fresh){try{const cards=await fetchHomeMarketCards(true);writeHomeMarketCache(cards);renderHomeMarketCards(grid,cards);}catch(e){console.error(e);if(!cached)grid.innerHTML='<p>Aktuální přehled indexů se nepodařilo načíst.</p>';}}clearInterval(homeMarketRefreshTimer);homeMarketRefreshTimer=setInterval(async()=>{const activeGrid=document.querySelector('#home-market-grid');if(!activeGrid)return;try{const cards=await fetchHomeMarketCards(true);writeHomeMarketCache(cards);renderHomeMarketCards(activeGrid,cards);}catch(e){console.warn('Desetiminutová aktualizace indexů selhala:',e);}},HOME_MARKET_CACHE_TTL_MS);}
